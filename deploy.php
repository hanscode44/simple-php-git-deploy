 ...
 *
 * @version 2.0
 * @link    https://github.com/hanscode44/simple-php-git-deploy
 */

// =========================================[ Configuration check ]===

/**
 * It's mandatory to configure the script using `deploy-config.php` file.
 *
 * Rename `deploy-config.example.php` to `deploy-config.php` and edit the
 * configuration options. That way, you won't have to edit the configuration
...
if (file_exists(basename(__FILE__, '.php') . '-config.php')) {
    ...
    die("<h2>No config<br>Add a config file to work with!</h2>");
}

// check if the Secret Access Token is changed,
if (!isset($_GET['sat']) || $_GET['sat'] !== SECRET_ACCESS_TOKEN) {
    header($_SERVER['SERVER_PROTOCOL'] . ' 403 Forbidden', true, 403);
    die('<h2>ACCESS DENIED!</h2>');
} else if (SECRET_ACCESS_TOKEN === 'BetterChangeMeNowOrSufferTheConsequences') {
    header($_SERVER['SERVER_PROTOCOL'] . ' 403 Forbidden', true, 403);
...
Running as <b><?php echo trim(shell_exec('whoami')); ?></b>.
    ...
    foreach ($requiredBinaries as $command) {
        ...
        if ($path == '') {
            ...
            die(
            sprintf(
                '<div class="error"><b>%s</b> not available. It needs to be installed on the server for this script to work.</div>',
                $command
            )
            );
        } else {
...
'
            ...
            if (EMAIL_ON_ERROR) {
                ...
                $headers = array();
                $headers[] =
                    sprintf('From: Simple PHP Git deploy script <simple-php-git-deploy@%s>', $_SERVER['HTTP_HOST']);
